...
3. **後続のセットアップ (development=true の場合)**
   ...
   `mise install` がネットワークや upstream 配布物の問題で失敗した場合でも、`chezmoi apply` 自体は継続します。失敗したランタイムは後で `mise install bun node python go rust usage npm:@openai/codex npm:ccusage` を手動で再実行してください。
